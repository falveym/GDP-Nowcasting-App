...
% Read the model specification workbook and normalize the header text.
% This helps the loader handle newer Excel files more reliably when the app
% passes in .xlsx workbooks instead of the original project files.
% ---------------------------------------------
raw = readcell(specfile);
raw = cellfun(@convertStringsToChars, raw, 'UniformOutput', false);
header = cellfun(@(x) strrep(convertStringsToChars(x),' ',''), raw(1,:), 'UniformOutput', false);
disp(header)
raw = raw(2:end,:);
...
% These columns are required by the nowcasting pipeline.
% If a required column is missing or renamed in the Excel spec file,
% the loader will stop with an error so the app does not run on a bad spec.
%------------------------------------------------------
...
% The block-loading structure determines how indicators enter the factor model.
% The first block is treated as the global block, so every modeled variable
% must load on Block 1.
%----------------------------------------------------
