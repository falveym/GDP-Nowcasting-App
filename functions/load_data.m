...
% Build the cached MAT-file path from the selected Excel file.
% This avoids brittle path assumptions and makes the loader more robust
% when files are chosen from different folders through the app UI.
datafile_mat = fullfile(folder, 'mat', [name '.mat']);
...
elseif any(strcmpi(ext,{'.xlsx','.xls'}))
    ...
    [matFolder,~,~] = fileparts(datafile_mat);
...
TEXT = cellfun(@convertStringsToChars, TEXT, 'UniformOutput', false);
Mnem = TEXT(1,2:end);
if ispc
...
for i = 1:N
    ...
    if isempty(idxSeriesSpec)
        error("SeriesID '%s' from Spec was not found in the data file headers.", Spec.SeriesID{i});
    elseif numel(idxSeriesSpec) > 1
        error("SeriesID '%s' appears multiple times in the data file headers.", Spec.SeriesID{i});
    end
